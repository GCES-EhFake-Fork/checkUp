...
class AgenciaBrasilSpider(BaseSpider):
    name = "agenciabrasilspider"
    start_urls = ["https://agenciabrasil.ebc.com.br/ultimas"]
    ...
    def allow_url(self, url: str) -> bool:
        # Validate domain
        if not url or "agenciabrasil.ebc.com.br" not in url:
            return False
        ...
        if len(segments) < 4:
            self.logger.info(f"URL with less than 4 segments: {url}")
            return False
