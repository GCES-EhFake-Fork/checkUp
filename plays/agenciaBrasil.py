...
class AgenciaBrasilPlay(BasePlay):
    name = "agenciabrasil"

    @classmethod
    def match(cls, url):
        return "agenciabrasil.ebc.com.br" in url
    ...
    def run(self) -> EntryItem:
        with sync_playwright() as p:
            ...
            description = ""
            try:
                description = page.locator("div.linha-fina-noticia").first.inner_text()
            except Exception as e:
            ...
            body = ""
            try:
                body_element = page.locator("div.conteudo-noticia")
                if body_element.count() > 0:
                    page.evaluate(
                        "document.querySelector('div.noticias-relacionadas')?.remove()"
                    )
            ...
            tags = []
            try:
                tag_elements = page.locator("div.row-tags a.tag")
